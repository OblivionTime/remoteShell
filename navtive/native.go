...
func StartProcessAsCurrentUser(appPath, cmdLine, workDir string) (uint32, error) {
	var (
		sessionId windows.Handle
	...
	if userToken, err = DuplicateUserTokenFromSessionID(sessionId); err != nil {
		return 0, fmt.Errorf("get duplicate user token for current user session: %s(, sessionID: %v)", err, sessionId)
	}
